...
def task_8_8():
    ...
    equation = rf"a={a};c={b}\quad\sqrt{{{amult**2}a^2 - {2*amult*bmult}ab + {bmult**2}b^2}}" 
...
def task_8_9():
    k = gen_digit()
    x = k * random.choice([2, 3, 5, 6, 9])
    equation = f"x={x}\\quad(x-{k}):\\frac{{x^2-{2*k}x+{k**2}}}{{x+{k}}}" 
    answer = (x + k) / (x - k)
    description = 'qwerty'
    return equation, answer, description
...
def task_8_10():
    a = gen_int_for_easy()
    k1 = gen_digit()
    while len(str((a+k1)/a)) >= 4:
        a = gen_int_for_easy()
        k1= gen_digit()
    k2 = gen_digit()
    equation = rf"a={a}\quad\frac{{a-{k2}}}{{a^2}} : \frac{{a-{k2}}}{{a^2+{k1}a}}" 
    answer = (a+k1)/a
    description = 'qwerty'
    return equation, answer, description
...
def task_8_11():
    a, b = gen_int_for_easy(), gen_int_for_easy()
    answer = a + b**2
    equation = rf"(\sqrt{{{a}}}+{b})^2-{2*b}\sqrt{{{a}}}" 
    description = 'qwerty'
    return equation, answer, description
...
def task_8_12():
    a, b = gen_int_for_easy(), gen_int_for_easy()
    k = gen_digit()
    equation = rf"a={a};b={b};\quad\frac{{{k}ab}}{{a+{k}b}}\cdot(\frac{{a}}{{{k}b}}-\frac{{{k}b}}{{a}})" 
    answer = a - k*b
    description = 'qwerty'
    return equation, answer, description
